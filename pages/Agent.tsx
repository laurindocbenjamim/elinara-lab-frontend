...
export const Agent: React.FC = () => {
    ...
    const handleDataSourceFieldChange = (field: string, value: any) => {
        if (field === 'config') {
            try {
                setFormData({ ...formData, config: JSON.parse(value) });
            } catch (e) {
                // Keep as string if invalid JSON for now, will validate on submit
                (formData as any).configRaw = value;
            }
        } else if (field === 'identifier') {
            setFormData({ ...formData, resource_identifier: value });
    ...
    const validateForm = () => {
        if (!formData.resource_identifier) return "Resource Identifier is required.";
        if (formData.platform === 'external_api' && !formData.resource_identifier.startsWith('http')) {
            try { new URL(formData.resource_identifier); } catch (e) { return `Invalid URL: ${formData.resource_identifier}`; }
        }
        if ((formData as any).configRaw) {
            try { JSON.parse((formData as any).configRaw); } catch (e) { return "Configuration must be a valid JSON array."; }
        }
        return null;
    };
    ...
    try {
        if (editingSourceId) {
            await dataSourcesService.update(editingSourceId, {
                ...
                resource_identifier: formData.resource_identifier,
            ...
            setMessage({ type: 'success', text: 'Data source added' });
        }
        fetchDataSources(processId);
...
const handleEditSource = (source: DataSource) => {
    ...
    setFormData({
        ...
        config: source.config || DEFAULT_CONFIG
    });
                            ...
                            <div className="p-6 flex-1 overflow-y-auto space-y-8">
                                <div>
                                    ...
                                    <div className="grid grid-cols-3 gap-2">
                                        {['google_drive', 'sharepoint', 'external_api'].map((p) => (
                                            <button
                                                key={p}
                                                onClick={() => handleDataSourceFieldChange('platform', p)}
                                ...
                                <div className="space-y-6">
                                    <div className="flex justify-between items-center">
                                        <label className="text-[10px] font-black text-gray-400 uppercase tracking-widest">Resource & Config</label>
                                    </div>

                                    <div className="p-4 rounded-2xl bg-gray-50 dark:bg-gray-900/50 border border-gray-100 dark:border-gray-700 space-y-4 relative group">
                                        <div>
                                            <label className="block text-[8px] font-black text-gray-400 uppercase tracking-widest mb-1">Resource Identifier</label>
                                            <input
                                                type="text"
                                                value={formData.resource_identifier}
                                                onChange={(e) => handleDataSourceFieldChange('identifier', e.target.value)}
                                        ...
                                        <div>
                                            <label className="block text-[8px] font-black text-gray-400 uppercase tracking-widest mb-1">Config (JSON Array)</label>
                                            <textarea
                                                value={typeof formData.config === 'object' ? JSON.stringify(formData.config, null, 2) : (formData as any).configRaw || ''}
                                                onChange={(e) => handleDataSourceFieldChange('config', e.target.value)}
